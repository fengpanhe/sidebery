...
这是为 Chrome、Brave 等 Chromium 浏览器编写的独立 Manifest V3 扩展，使用浏览器原生侧边栏。界面采用浅薄荷背景（`#F1FFFE`）、彩色分组标签和白色选中项。它实现常用的侧边栏标签管理功能，并不是 Firefox 版全部功能的完整移植。
...
- 拖拽调整顺序和父子关系（上沿放到前面、下沿放到分支后面、中间作为子标签）。
...
- `npm run test.chromium`：36 个自动化测试通过，覆盖树循环和边界校验、后台唤醒、标签 ID 替换、折叠分支关闭、带来源的新标签子级关系与外部链接置尾、复制和跨窗口移动、快照校验和恢复、多选、错误提示、样式更新及极简标签页布局。
